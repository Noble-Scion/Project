...
public class UserDetailsController {
    ...
    @PostMapping("/details/{userId}/car")
    public ResponseEntity<?> updateCarDetails(@PathVariable Long userId, @RequestBody CarDTO carDTO) {
        try {
            userDetailsService.updateCarDetails(userId, carDTO);
            return ResponseEntity.ok().build();
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
